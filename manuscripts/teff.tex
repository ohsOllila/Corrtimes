...
\todo{SAMULI: I think that we should define the criteria for selecting the analyzed simulations here.}
...
\todo{SAMULI: I would emphasize here more the automatization of the analysis, and also mention that the mapping file was used
to enable automatic analysis of simulations with different force fields having different atom names (if this was the case?).}
...
\todo{SAMULI: I do not understand this:}
...
%by integrating $g'_\mathrm f(\tau)$,
%see
from Eq.~\eqref{eq:teff} %and~\eqref{eq:nBCF},
...
for each individual lipid $m$, %over the whole trajectory,
and then obtaining the error estimate
$\Delta g_{\mathrm{f}}(\tau)$
as the standard error of the mean over the $N_\mathrm l$ lipids
%
%Importantly, this gives an uncertainty estimate
...
\todo{SAMULI: I think that we can say stronger than "seems feasible to assume" because experiments in Tiago's 2015 paper indicate that these,
or maybe even shorter, timescales are not present. Maybe Tiago can comment this?}
...
\end{equation}
%
\todo{SAMULI: I think that below text may sound weird for NMR people.
  I think that bonds do not have $R_{1}$ rates, nucleus has.
  I think that this should be rewritten in the way that
  we talk about 13C spin relaxation from C-H coupling in the
  way that it still looks like that we understand that this is actually a spin system.
  This is not easy, and right now I cannot figure out an easy way to it.
  I can think this later if needed.
}
...
%
\todo{SAMULI: When does this happen:}
...
\todo{SAMULI: Do we need this paragraph at all?}
...
\subsection*{Internal lipid dynamics in POPC bilayer compared with NMR experiments}
The internal dynamics of POPC lamellar liquid crystalline state is compared between
MD simulations and NMR experimentsin Fig.~\ref{fig:teff_R1} using the effective
correlation time $\tau_\mathrm{e}$ and $R_{1}$ spin relaxation rate.
%obtained for fully
%hydrated POPC bilayers in experiments (black) and in the six different MD force fields (color).
...
In line with the comparisons of conformational ensembles using order parameters \cite{ollila16},
the effective correlation times indicate that the average dynamics of hydrophopic acyl chain region is
better reproduced in MD simulation models than of glycerol backbone and headgroup. 
%Qualitatively, every force field captures
All force fields qualitatively capture the slower dynamics in glycerol backbone than in the headgroup and the tails,
%The general shape of the $\tau_\mathrm{e}$ profile
%is d by all,
but
%Quantitatively,
MD simulations have a tendency towards
slightly too fast dynamics in the membrane core and
too slow dynamics at the water-facing interface.% MD is typically too slow.
The detected slow glycerol backbone dynamics in MD is consistent with previous results for the Berger model~\cite{ferreira15}
%. It also agrees with the  insufficient
\todo{SAMULI: I am not sure if we should be mention this work here, because we do not have data for CHARMMc32b2:}
and with very slow conformational sampling of glycerol backbone torsions observed in 500-ns-long CHARMMc32b2~\cite{schlenkrich96,feller00} simulations of a DOPC lipid~\cite{vogel12}.
% The first carbon of the palmitoyl tail is the only location where some force fields have a tendency to underestimate $\tau_{\mathrm{eff}}$. 
Such general differences between MD simulations and experiments are not visible in the $R_1$ rates that mainly detect
dynamics at $\sim 1-10$ ns timescales.
...
This is not surprising because CHARMM36 reproduces also the most realistic conformational ensembles,
while Slipids does not correctly capture glycerol backbone structures \cite{botan15,antila19}.
%%
...
%This means that the
conformational ensembles of glycerol backbone and headgroup 
%simulated do not
greatly differ between MD simulation force fields and are not exactly correct in ay of them \cite{botan15,antila19}.
%match
...
%
%Quantitatively,
there are a few carbons segments in the data for which the experimental order parameters, $R_1$ and $\tau_\mathrm{e}$ are approximately reproduced by simulations,
suggesting that the conformational ensemble and dynamics is correctly captured by MD simulatios in these cases. 
%(almost) correct rotational dynamics at all relevant time scales.
%
%, and for the .
%
For example, 
...
There are also cases where order parameters and $\tau_\mathrm{e}$ agree with experiments at least approximately,
but $R_1$ does not such as ---
%is demonstrated by all five all-atom force fields for the $\gamma$ segment,
%for Slipids around the double bond,
%such as
$\beta$ and $\alpha$ carbons in CHARMM36 force field.
Therein a cancellation of error occurs in $\tau_\mathrm{e}$: The overestimated relative dynamics at the 1\,ns scale are compensated by wrong dynamics at the other time scales.
...
%%
%%  SAMULI: I commented this section out
%%
...
The opposite situation is observed for the $\beta$, $\alpha$, g$_3$, and g$_1$ segments in MacRog simulation
%provides a prominent example
where the $R_1$ rates are well reproduced, but $\tau_\mathrm e$ times systematically overestimated.
Such a combination suggests that MD correct relative weight of 1\,ns scale dynamics, but has too slow long-time dynamics.
...
\todo{SAMULI: I would move this to methods.}
...
\subsection*{Internal motions of POPC headgroup in MD simulations}
Because lipid headgroups facing toward water phase are the first membrane parts that interact
with approaching proteins, understanding their conformations and dynamics is crucial to
explain biological processes where proteins binding to membranes plays critical role.
Yet, the analysis of lipid headgroup dynamics from MD simulations and NMR experiments has gained
much less attention than acyl chains \cite{ollila16,??}. Therefore, we focus here to discuss
lessons learned on lipid headgroup dynamics from the analysis of the databank.
...
while Slipids provides almost perfect dynamics for $\beta$ and $\alpha$ carbons.
Therefore, Slipids simulation gives a realistic model for the conformational dynamics
in this region, but relative weight of timescales around $\sim$1 ns are overestimated
in CHARMM36 simulations.

To further investigate the internal timescales of lipids in simulations,
we plot the distribution of relative weights, $\alpha_i$ in Eq.~\eqref{eq:weights},
resulting from the fitting to the rotational correlation functions from simulatons 
%This also manifests in the
%distribution of fitting weights ($\alpha_i$ in Eq.~\eqref{eq:weights})
in Fig.~\ref{fig:cumulativeR1s}C, and their 
%where the differences between force fields arise, we visualize the 
'cumulative' contribution to $R_1(\tau)$ in Fig.~\ref{fig:cumulativeR1s}B.
\todo{SAMULI: maybe change the order of B and C in figure~\ref{fig:cumulativeR1s}?}

\todo{SAMULI: Maybe this to methods:}
...
%
% SAMULI: I have commented this paragraph out
%
...
The dominant relative timescales with the largest weight factors, $\alpha_i$,
are below 100 ps in all models, except in CHARMM36.
The large relative weights between 200~ps and 2~ns
explain the overestimated $R_1$ rates
%closer to $\sim$1ns timescales that mostly
%contribute to the 
for the $\beta$ and $\alpha$ segments in CHARMM36 in Fig.~\ref{fig:cumulativeR1s}.
%shows
...
%
%For example
All the other models reproduce $R_1$ rates closer to experiments for
these segments, while only Slipids simultaneously gives the correct $\tau_\mathrm e$.
%correct,
%has its largest weights at $\tau<200$\,ps.
%
The overestimated $\tau_\mathrm e$ values in MacRog and Berger simulations
can be explained by the relative contributions from long time scales
around $\sim ??$\,ns. \todo{SAMULI: I think we could mention here the approximative location of the peak maximum,
  but it is hard to see now because not all x-axis values are there.} % in both $\beta$ and $\alpha$ for MacRog and Berger)
On the other hand, the underestimated $\tau_\mathrm e$ values in Lipid14, ECC, Berger
simulations can be explained by significant relative contributions from 
%Indeed, considerable weights
short time scale dynamics below $\sim20$\,ps.% in $\alpha$ for Lipid14, ECC, Berger) and
Such short and long time scale contributions are not present in Slipids that
correctly reproduces the experimental values for both $\tau_\mathrm e$ and $R_1$,
suggesting the dynamics of $\alpha$ and $\beta$ segments in POPC headgroup occurs
between $\sim$20 ps - 10 ns with the dominant timescales between 40-70 ps.
...
It would be highly interesting to identify the origin of observed artificial timescales,
particularly for the 0.2--2\,ns window that are over-presented in CHARMM36, and
propose how those could be corrected in the simulation models.
%observed in simulations models, and especially the 
...
show that 
the conformational dynamics of glycerol backbone and acyl chains
%region
slow down markedly when cholesterol is added.
%
...
All the force fields investigated qualitatively reproduce the increase in $\tau_\mathrm e$ in acyl chain region (see Fig.~\ref{fig:chol}B):
Slipids gives the best magnitude estimates, while CHARMM36 and MacRog clearly overestimate the changes at the glycerol, C2, and C3 carbons.
Notably, MacRog \todo{and Berger?} erroneously predict slow down also for the $\beta$ and $\alpha$ carbons in headgroup, for which experiments detect no change.
%
Note that,  while CHARMM36 correctly shows no chance in $\tau_\mathrm{e}$
of the headgroup $\gamma$, $\beta$, and $\alpha$ carbons,
...
In conclusion, the experiments suggest that acyl chain ordering upon cholesterol addition is accomppanied
with slower internal dynamics in hydrophobic core and glycerol backbone region, while headgroup dynamics is almost
intact even with 50\% of cholesterol, supporting the previous ideas that acyl chains and headgroup can respond
...
in acyl chain region rather well, but changes in headgroup and glycerol backbone are often overestimated.

\subsection*{Effect of drying.} \todo{SAMULI: I would use dehydration instead of drying.}
Membrane fusion is always preceded by dehydration of the water between opposing membranes.
Lipid bilayers in low hydrated conditions are also found, for example, from skin tissue.
Therefore, the influence of dehydration of lipid bilayers may play a role, for example,
also in the rate of synaptic vesicle fusion and drying of skin. 
...
SAMULI: I agree with Markus. If kept, I would focus on calcium data from most realistic models, i.e., ECC.}
...
\section{Conclusions}
\todo{SAMULI: I do not have time formulate but I think we should emphasize that for some segments we get order parameter, effective CT and $R_1$
  correctly, meaning that conformational ensemble and dynamics is possible to capture. Also I would emphasize the results from cholesterol that headgroup dynamics
is intact.}
